 ...
 */
export default function serveStatic(path: string, options?: IBaseOptions): (req: Request) => Promise<Response>;
 ...
 * @param options Static files options (indexFile defaults to "index.html")
 */
export default function serveStatic(path: string, options: IMiddlewareOptions): (ctx: Context) => Promise<Context>;
...
export default function serveStatic(path: string, options: TOptions) {
	...
	async function getResponse(req: Request): Promise<Response> {
		...
		if (existsSync(`${filepath}/${indexFile}`)) {
			...
			return new Response(file, {
				headers: {
					"Content-Type": `${mime.getType(`${filepath}/${indexFile}`)}; charset=utf-8`,
				},
			});
		}
